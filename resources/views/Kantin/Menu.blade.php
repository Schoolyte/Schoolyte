...
<body>
    <div class="menu" style="min-height: 270vh;">
        ...
        <h1 style="text-align:center">{{ $stand->nama_stand }}</h1>
        <h1 style="text-align:center"><br>Kode : {{ $stand->id }} </h1>
       <span class="jenis"><br><br>{{ $stand->jenis_stand }}  <hr></span><br>
        <form>
            <input type="hidden" name="action" value="rincian">
            @foreach ($menus as $item)
                <div class="menu{{$loop->index + 1}}">
                    ...
                    <div class="pesanan">{{ $item->nama_menu }}</div>
            ...
            <button type="submit" style="position: absolute; top: 1200px; left: 10vh;padding: 1rem;background: rgb(255, 78, 65); width: 1300px;border-radius: 10px;font-size: 1rem; color: white; text-align: center; cursor: pointer; border:none">
                BELI
            </button>
        </form>
    </div>
